...
get '/' => sub {
    ...
    my @dancerlicious_versions = _get_dancerlicious_versions( \@perl_versions );

    my @combis = map{ my $perl = $_; map{ "$perl / $_" }@dancerlicious_versions }@perl_versions;

    my $db  = _connect_db();
    my $sth = $db->prepare( 'SELECT pname, pversion, abstract, perl_version, dancer_version, result, author FROM matrix' );
    $sth->execute;

    my %results = ( 0 => 'nok', 1 => 'ok', -1 => 'requires greater version of Dancer2' );
    ...
    my $html = $self->render_to_string(
        ...
        combis  => \@combis,
        plugins => \%plugins,
        dancers   => \@dancerlicious_versions,
        perls   => \@perl_versions,
    );
...
<html>
  <head>
    <title>Dancer2-Plugins : Installable</title>
...
a("#"+c+" tbody tr").each(function(){var b=a(this).html().toLowerCase().replace(/<.+?>/g,"").replace(/\s+/g," "),c=0;a.each(d,function(){if(0>b.indexOf(this))return c=1,!1});c?a(this).hide():a(this).show()})},300));return this};a.fn.addTableFilter.defaults={labelText:"Keyword(s): ",size:32};a.fn.delayBind=function(d,b,c,e){a.isFunction(b)&&(e=c,c=b,b=void 0);var g=this,f=null;return this.bind(d,b,function(b){clearTimeout(f);f=setTimeout(function(){c.apply(g,[a.extend({},b)])},e)})}})(jQuery);
  ...
  <body>
    <div class="filter">
% my $latest_dancer = $dancers->[-1];
% $latest_dancer =~ tr/./-/;

       Dancer2:
% for my $index ( 0 .. $#{ $dancers } ) {
%     my $dancer       = $dancers->[$index];
%     my $latest     = $index == $#{$dancers} ? 'checked="checked"' : '';
%     (my $dancer_value = $dancer) =~ tr/./-/;
         <input type="checkbox" name="filter" value="<%= $dancer_value %>" <%== $latest %> /><%= $dancer %>
% }
       Perl:
    ...
    <script type="text/javascript">
        $(document).ready( function() {
           ...
           if ( terms[0] !== '' ) {
               ...
               if ( !checkboxes ) {
                   var checkbox = $('input[value="<%= $latest_dancer %>"]').prop( 'checked', true );
               }
