...
sub create_matrix {
    ...
    my $sth  = $db->prepare( 'INSERT INTO matrix (pname, pversion, abstract, perl_version, dancer_version, result, author, error_log) VALUES( ?,?,?,?,?,?,?,? )' );
        ...
        for my $perl ( @{ $perls } ) {
            ...
            for my $dancer ( @{ $dancers } ) {
                ...
                if ( $cpanm_output =~ m{Successfully installed Dancer2-\d+} ) {
                    ...
                    $sth->execute( $module, $info->{version}, $info->{abstract}, $perl, $dancer, 0, $info->{author}, $log // '' );
...
sub _find_or_create_db {
    ...
    if ( !$exists ) {
        my @creates = (
            q~CREATE TABLE matrix ( pname TEXT NOT NULL, pversion TEXT NOT NULL, abstract TEXT, perl_version TEXT NOT NULL, dancer_version TEXT NOT NULL, result TEXT, author TEXT, error_log TEXT )~,
        );
